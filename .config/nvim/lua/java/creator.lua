...
local function detect_current_package()
  ...
  if not src_path then
    -- Si no estamos en un archivo Java, usar src/main/java por defecto
  ...
  if package_path then
    ...
    -- Si no hay archivo actual, intentar detectar desde el directorio
...
function M.create_java_item(type)
  ...
  local suggested_package, src_path = detect_current_package()

  -- 2. Preguntar por el package (con sugerencia)
  vim.ui.input({
    prompt = "Package: ",
    ...
    -- 3. Preguntar por el nombre de la clase/interface/etc
    vim.ui.input({
...
function M._write_file(type, name, package_name, file_dir, file_path)
  ...
  -- 6. Generar contenido según el tipo
  ...
  if file then
    ...
    -- 9. Posicionar cursor dentro de la clase (línea 4, después de la declaración)
...
function M.move_to_package()
  ...
  vim.ui.input({
    ...
    if new_file then
      ...
      vim.cmd("bdelete!") -- Cerrar buffer actual
      vim.fn.delete(old_file_path) -- Eliminar archivo viejo
